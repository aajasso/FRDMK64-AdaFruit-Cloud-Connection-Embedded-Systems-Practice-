...
#define EXAMPLE_MQTT_CLIENT_ID "k64"
 ...
 // #define LOCAL_BROKER para dispositivos local
...
#define EXAMPLE_MQTT_USER "aajasso"
#define EXAMPLE_MQTT_PSWD "aio_jXHr52Iv8uvBazLr6uZmD5e6YH2B" // llave del dashboard

/*! @brief MQTT server port number. */
#define EXAMPLE_MQTT_SERVER_PORT 1883 // puerto
...
static void publish_stream(void *ctx);
...
static void mqtt_incoming_data_cb(void *arg, const u8_t *data, u16_t len, u8_t flags)
{
    ...
    {
    	PRINTF("\\x%02x", data[i]); // imprime datos si no es on o off
...
static void mqtt_subscribe_topics(mqtt_client_t *client)
{
    ...
    static const char *topics[] = {"aajasso/feeds/boton", "aajasso/feeds/texto", "aajasso/feeds/grafica"};
    int qos[]                   = {1,1,1}; // quality of service
    err_t err;
    int i;
...
static void publish_stream(void *ctx)
{
	PRINTF(" Publisher  2 Stream ");
    static const char *topic   = "aajasso/feeds/stream";
    static char message[10];

    LWIP_UNUSED_ARG(ctx);

    memset(message, 0, 10);
    sprintf(message, "%d", stream);
...
static void publish_humidity(void *ctx)
{
    static const char *topic   = "aajasso/feeds/humedad";
    static char message[10];
...
static void publish_indicator(void *ctx)
{
    static const char *topic   = "aajasso/feeds/indicator";
    static char message[10];
...
static void app_thread(void *arg)
{
    ...
    while(1) {
		...
		if(uxBits & MQTT_CONNECTED_EVT ) {
			...
			err = tcpip_callback(publish_stream, NULL); // publish 2
		}
			...
			if(sprinklers_on){
				...
				indicator = true;
				PRINTF("Indicator: TRUE\r\n");
			}
			else {
				indicator = false;
				PRINTF("Indicator: FALSE\r\n");
				GPIO_PortSet(BOARD_LED_GPIO, 1u << BOARD_LED_GPIO_PIN);
			}
			err = tcpip_callback(publish_indicator, NULL);
